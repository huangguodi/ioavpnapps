...
object Bridge {
    ...
    init {
        ...
        // Align Home with Flutter's getApplicationDocumentsDirectory (app_flutter)
        // ctx.filesDir is ".../files". We want ".../app_flutter".
        val home = ctx.filesDir.parentFile?.resolve("app_flutter")?.apply { mkdirs() }?.absolutePath
            ?: ctx.filesDir.resolve("clash").apply { mkdirs() }.absolutePath
        val versionName = ctx.packageManager.getPackageInfo(ctx.packageName, 0).versionName ?: "1.0.0"
