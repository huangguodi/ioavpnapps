...
5. 自动换算并写入 `orders.description` 字段：`年->365`、`半年->180`、`季->90`、`月->30`。
6. 回调成功时流量日志有效期 `user_traffic_log.valid_days` 使用 `orders.description` 的换算值。
7. 业务处理参考 `/app/v1/user/order/checkout`：校验支付通道与订单归属，创建支付并返回支付地址。
